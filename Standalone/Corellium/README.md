# Installing GreenSn0w on a Corellium
...
2. Compile GreenSn0w
3. Run `codesign -dvvvv on BaseBin/greensn0w/greensn0w`
4. Copy the CandidateCDHash and paste it into Settings -> Trust Cache on the Non-Jailbroken Corellium instance
5. Run `env NODE_TLS_REJECT_UNAUTHORIZED=0 node greensn0w.js install <Project> <Instance>` where `Project` and `Instance` can either be a name or an ID
