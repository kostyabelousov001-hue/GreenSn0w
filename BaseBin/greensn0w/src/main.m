...
void install_greensn0w(void)
{
	...
	printf("GreenSn0w installed!\n");
}

void install_greensn0w_from_tarball(const char *tarballPath, const char *bootstrapPath)
{
	[[NSFileManager defaultManager] removeItemAtPath:@"/private/preboot/greensn0w_tmp" error:nil];
	[[NSFileManager defaultManager] createDirectoryAtPath:@"/private/preboot/greensn0w_tmp" withIntermediateDirectories:NO attributes:nil error:nil];
	libarchive_unarchive(tarballPath, "/private/preboot/greensn0w_tmp");
	if (bootstrapPath) {
		[[NSFileManager defaultManager] copyItemAtPath:[NSString stringWithUTF8String:bootstrapPath] toPath:@"/private/preboot/greensn0w_tmp/bootstrap_1900.tar.zst" error:nil];
	}

	const char *newPath = "/private/preboot/greensn0w_tmp/greensn0w";
	extern char **environ;
	char *argv[] = (char *[]){
		(char *)newPath,
		(char *)"install",
...
void activate_greensn0w(void)
{
	...
	printf("GreenSn0w activated, see you on the other side!\n");
}
	...
	if (argc >= 2) {
		if (!strcmp(argv[1], "install")) {
			if (argc >= 3) {
				...
				install_greensn0w_from_tarball(argv[2], bootstrapPath);
			}
			else {
				install_greensn0w();
			}
