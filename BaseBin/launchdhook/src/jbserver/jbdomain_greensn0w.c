...
bool greensn0w_domain_allowed(audit_token_t clientToken)
{
	char path[PATH_MAX];
	if (proc_pidpath_audittoken(&clientToken, path, PATH_MAX) <= 0) return false;
	return is_greensn0w_app(path);
}

bool greensn0w_is_jailbroken(char **outVersion)
{
	*outVersion = read_file_to_string(JBROOT_PATH("/basebin/.version"));
	return true;
}

int greensn0w_get_root(audit_token_t *processToken)
{
...
int greensn0w_drop_root(audit_token_t *processToken)
{
	pid_t pid = audit_token_to_pid(*processToken);
	uint64_t proc = proc_find(pid);
	uint64_t ucred = proc_ucred(proc);
...
struct jbserver_domain gGreenSn0wDomain = {
	.permissionHandler = greensn0w_domain_allowed,
	.actions = {
		// JBS_DOPAMINE_IS_JAILBROKEN
		{
			.handler = greensn0w_is_jailbroken,
			.args = (jbserver_arg[]){
