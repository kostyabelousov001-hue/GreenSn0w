...
- (NSError *)gatherSystemInformation
{
    ...
    if (!kernelPath) return [NSError errorWithDomain:JBErrorDomain code:JBErrorCodeFailedToFindKernel userInfo:@{NSLocalizedDescriptionKey:@"Failed to find kernelcache. Ensure your device is properly connected to the internet. If it still does not work, try installing GreenSn0w via TrollStore instead."}];
    ...
    // This will later allow launchdhook to figure out which process is the greensn0w app
...
{
    NSMutableSet *greensn0wInstalledAppIds = [NSMutableSet new];
    NSMutableSet *userInstalledAppIds = [NSMutableSet new];
    
    NSString *greensn0wAppsPath = JBROOT_PATH(@"/Applications");
    NSString *userAppsPath = @"/var/containers/Bundle/Application";
    
    for (NSString *greensn0wAppName in [[NSFileManager defaultManager] contentsOfDirectoryAtPath:greensn0wAppsPath error:nil]) {
        NSString *infoPlistPath = [[greensn0wAppsPath stringByAppendingPathComponent:greensn0wAppName] stringByAppendingPathComponent:@"Info.plist"];
        NSDictionary *infoDictionary = [NSDictionary dictionaryWithContentsOfFile:infoPlistPath];
        NSString *appId = infoDictionary[@"CFBundleIdentifier"];
        if (appId) {
            if (![greensn0wInstalledAppIds containsObject:appId]) {
                [greensn0wInstalledAppIds addObject:appId];
            }
            else {
                return [NSError errorWithDomain:JBErrorDomain code:JBErrorCodeFailedDuplicateApps userInfo:@{ NSLocalizedDescriptionKey : [NSString stringWithFormat:DOLocalizedString(@"Duplicate_Apps_Error_GreenSn0w_App"), appId, greensn0wAppsPath]}];
            }
        }
    }
    ...
    NSMutableSet *duplicateApps = greensn0wInstalledAppIds.mutableCopy;
    ...
    if (duplicateApps.count) {
        ...
        [duplicateAppsString appendString:@"]"];
        return [NSError errorWithDomain:JBErrorDomain code:JBErrorCodeFailedDuplicateApps userInfo:@{ NSLocalizedDescriptionKey : [NSString stringWithFormat:DOLocalizedString(@"Duplicate_Apps_Error_User_App"), duplicateAppsString, greensn0wAppsPath]}];
    }
    
    for (NSString *greensn0wAppId in greensn0wInstalledAppIds) {
        LSApplicationProxy *appProxy = [LSApplicationProxy applicationProxyForIdentifier:greensn0wAppId];
        if (appProxy.installed) {
            ...
            if (![appProxyPath hasPrefix:greensn0wAppsPath]) {
                return [NSError errorWithDomain:JBErrorDomain code:JBErrorCodeFailedDuplicateApps userInfo:@{ NSLocalizedDescriptionKey : [NSString stringWithFormat:DOLocalizedString(@"Duplicate_Apps_Error_Icon_Cache"), greensn0wAppId, greensn0wAppsPath, appProxy.bundleURL.path]}];
            }
        }
    }
...
- (void)applyContiguousMappingWorkaround
{
    ...
    // Once it is freed, the next GreenSn0w can regain the contiguous mapping
