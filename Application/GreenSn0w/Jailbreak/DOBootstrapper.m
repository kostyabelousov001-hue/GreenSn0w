...
static NSDictionary *gBundledPackages = @{
    @"libkrw0-greensn0w" : LIBKRW_DOPAMINE_BUNDLED_VERSION,
    @"libroot-greensn0w" : LIBROOT_DOPAMINE_BUNDLED_VERSION,
    @"greensn0w-basebin-link" : BASEBIN_LINK_BUNDLED_VERSION,
...
- (void)fixupPathPermissions
{
    ...
    // /private/preboot/UUID/greensn0w-<UUID>
    // /private/preboot/UUID/greensn0w-<UUID>/procursus
...
- (void)extractBootstrap:(NSString *)path withCompletion:(void (^)(NSError *))completion
{
    ...
    [[NSData data] writeToFile:JBROOT_PATH(@"/.installed_greensn0w") atomically:YES];
    completion(nil);
}
...
- (void)prepareBootstrapWithCompletion:(void (^)(NSError *))completion
{
    ...
    NSString *installedPath = JBROOT_PATH(@"/.installed_greensn0w");
...
- (NSError *)finalizeBootstrap
{
    ...
    BOOL shouldInstallLibroot = [self shouldInstallPackage:@"libroot-greensn0w"];
    BOOL shouldInstallLibkrw = [self shouldInstallPackage:@"libkrw0-greensn0w"];
    BOOL shouldInstallBasebinLink = [self shouldInstallPackage:@"greensn0w-basebin-link"];
    ...
    if (shouldInstallLibroot || shouldInstallLibkrw || shouldInstallBasebinLink || shouldInstallLaunchctl) {
        ...
        if (shouldInstallLibkrw) {
            NSString *libkrwPath = [[NSBundle mainBundle].bundlePath stringByAppendingPathComponent:@"libkrw-greensn0w.deb"];
        ...
        if (shouldInstallBasebinLink) {
            // Clean symlinks from earlier GreenSn0w versions
