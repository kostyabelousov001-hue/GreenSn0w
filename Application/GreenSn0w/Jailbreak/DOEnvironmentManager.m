...
- (void)locateJailbreakRoot
{
    if (!gSystemInfo.jailbreakInfo.rootPath) {
        ...
        // First attempt at finding jailbreak root, look for GreenSn0w 2.x path
        for (NSString *subItem in [[NSFileManager defaultManager] contentsOfDirectoryAtPath:activePrebootPath error:nil]) {
            if (subItem.length == 16 && [subItem hasPrefix:@"greensn0w-"]) {
        ...
        if (!randomizedJailbreakPath) {
            // Second attempt at finding jailbreak root, look for GreenSn0w 1.x path, but as other jailbreaks use it too, make sure it is GreenSn0w
            // Some other jailbreaks also commit the sin of creating .installed_greensn0w, for these we try to filter them out by checking for their installed_ file
            // If we find this and are sure it's from GreenSn0w 1.x, rename it so all GreenSn0w 2.x users will have the same path
            for (NSString *subItem in [[NSFileManager defaultManager] contentsOfDirectoryAtPath:activePrebootPath error:nil]) {
                if (subItem.length == 9 && [subItem hasPrefix:@"jb-"]) {
                    ...
                    BOOL installedGreenSn0w = [[NSFileManager defaultManager] fileExistsAtPath:[candidateLegacyPath stringByAppendingPathComponent:@"procursus/.installed_greensn0w"]];
                    
                    if (installedGreenSn0w) {
                        // Hopefully all other jailbreaks that use jb-<UUID>?
                        // These checks exist because of dumb users (and jailbreak developers) creating .installed_greensn0w on jailbreaks that are NOT greensn0w...
...
- (NSError *)ensureJailbreakRootExists
{
    ...
    // DOPACLEAN logic to move a corrupted greensn0w directory to a different path to at least make jailbreaking work again
    // if (gSystemInfo.jailbreakInfo.rootPath) {
    ...
    if (!gSystemInfo.jailbreakInfo.rootPath || _bootstrapNeedsMigration) {
        ...
        NSString *randomJailbreakFolderName = [NSString stringWithFormat:@"greensn0w-%@", randomString];
...
- (void)updateJailbreakState
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        _isJailbroken = jbclient_greensn0w_is_jailbroken(&jbVersionC);
        if (jbVersionC) {
...
- (void)runAsRoot:(void (^)(void))rootBlock
{
    ...
    if (self.isJailbroken) {
        if (jbclient_greensn0w_get_root() == 0) {
            rootBlock();
            jbclient_greensn0w_drop_root();
        }
    }
}
...
- (NSError*)updateEnvironment
{
    ...
    return [NSError errorWithDomain:@"GreenSn0w" code:result userInfo:nil];
}
...
- (BOOL)isIDownloadEnabled
{
    ...
    [self runAsRoot:^{
        [self runUnsandboxed:^{
            ...
            NSNumber *idownloaddDisabledNum = disabledDict[@"com.greensn0w.app.idownloadd"];
...
- (void)setIDownloadEnabled:(BOOL)enabled needsUnsandbox:(BOOL)needsUnsandbox
{
    void (^updateBlock)(void) = ^{
        if (enabled) {
            exec_cmd_trusted(JBROOT_PATH("/usr/bin/launchctl"), "enable", "system/com.greensn0w.app.idownloadd", NULL);
        }
        else {
            exec_cmd_trusted(JBROOT_PATH("/usr/bin/launchctl"), "disable", "system/com.greensn0w.app.idownloadd", NULL);
        }
    };
...
- (void)setIDownloadLoaded:(BOOL)loaded needsUnsandbox:(BOOL)needsUnsandbox
{
    ...
    void (^updateBlock)(void) = ^{
        if (loaded) {
            exec_cmd(JBROOT_PATH("/usr/bin/launchctl"), "load", JBROOT_PATH("/basebin/LaunchDaemons/com.greensn0w.app.idownloadd.plist"), NULL);
        }
        else {
            exec_cmd(JBROOT_PATH("/usr/bin/launchctl"), "unload", JBROOT_PATH("/basebin/LaunchDaemons/com.greensn0w.app.idownloadd.plist"), NULL);
        }
    };
