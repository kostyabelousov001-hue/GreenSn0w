...
- (NSArray *)getLatestReleases
{
    ...
    dispatch_once(&onceToken, ^{
        NSURL *url = [NSURL URLWithString:@"https://api.github.com/repos/kostyabelousov001-hue/GreenSn0w/releases"];
...
- (void)sendLog:(NSString*)log debug:(BOOL)debug update:(BOOL)update
{
    ...
    BOOL isDebug = self.logView.class == DODebugLogView.class || self.logView.class == DOTerminalLogView.class;
    if (debug && !isDebug) {
        [_logLock unlock];
        return;
    }
